...
class IntegratedVideoDataset(Dataset):
    ...
    def _run_da3_geometry(self, model_path):
        ...
        if hasattr(prediction, 'aux') and prediction.aux is not None:
            ...
            if key in prediction.aux: da3_features = prediction.aux[key]
        if da3_features is None and hasattr(prediction, 'int_feats'):
             da3_features = prediction.int_feats[0]
        
        if da3_features is None: raise ValueError("Could not find features in DA3 output!")
        if not isinstance(da3_features, torch.Tensor): da3_features = torch.from_numpy(da3_features).to(self.device)
        else: da3_features = da3_features.to(self.device)
            
        if da3_features.ndim == 5: da3_features = da3_features.squeeze(0) 
        ...
        all_pts, all_feats, all_rgb, all_times = [], [], [], []
        ...
        for i in range(self.num_frames):
            ...
            if idx.numel() > 30000: idx = idx[torch.randperm(len(idx))[:30000]]
            ...
            sampled_feat = torch.nn.functional.grid_sample(feat_map, grid, align_corners=True, mode='bilinear')
            sampled_feat = sampled_feat.squeeze().permute(1, 0)
            
            sampled_rgb = img_tensor.flatten(0, 1)[idx] 
            combined_feat = torch.cat([sampled_feat, sampled_rgb], dim=-1)
            
            all_feats.append(combined_feat.cpu())
            all_rgb.append(sampled_rgb.cpu())
            
            # [新增] 记录每个点的时间戳
            # shape: [N_points, 1]
            time_col = torch.full((pts_w.shape[0], 1), curr_time, device='cpu', dtype=torch.float32)
            all_times.append(time_col)

        raw_coords = torch.cat(all_pts, 0)
        big_feats = torch.cat(all_feats, 0) # CPU
        big_rgb = torch.cat(all_rgb, 0)     # CPU
        big_times = torch.cat(all_times, 0) # CPU
        ...
        norm_coords = (raw_coords - scene_center) * scale
        
        # === Voxel Downsampling ===
        print(f"   [Info] Raw points: {norm_coords.shape[0]}")
        grid_res = 0.015 
        quantized = torch.round(norm_coords / grid_res).long()
        
        primes = np.array([73856093, 19349663, 83492791], dtype=np.int64)
        hashes = (quantized.cpu().numpy() * primes).sum(axis=-1)
        _, unique_indices_np = np.unique(hashes, return_index=True)
        
        unique_indices_cuda = torch.from_numpy(unique_indices_np).to(self.device)
        unique_indices_cpu = torch.from_numpy(unique_indices_np).to('cpu')
        
        print(f"   [Info] Unique points: {unique_indices_cuda.shape[0]}")
        if unique_indices_cuda.shape[0] > 30000: # 稍微放宽一点限制
             perm = torch.randperm(unique_indices_cuda.shape[0])[:30000]
             unique_indices_cuda = unique_indices_cuda[perm.to(self.device)]
             unique_indices_cpu = unique_indices_cpu[perm.to('cpu')]
        
        self.big_coords = norm_coords[unique_indices_cuda] 
        self.big_feats = big_feats[unique_indices_cpu] 
        self.big_rgb = big_rgb[unique_indices_cpu]
        # [新增] 保存降采样后的时间戳
        self.big_times = big_times[unique_indices_cpu]
        
        print(f"   [Info] Final points: {self.big_coords.shape[0]}")
    ...
    def _run_concerto(self, model_path, voxel_size):
        ...
        with torch.no_grad():
            ...
            print("   Matching Visual features to Concerto tokens...")
            ...
            # [新增] 同时也把 Time 信息匹配给 Concerto Tokens
            self.scene_times = self.big_times[indices.flatten()].cpu()
    ...
    def _extract_gt_features_dinov2(self, model_path):
        print(f">> [3/3] Extracting GT Features...")
        try:
        ...
        for p in self.image_paths:
            ...
            with torch.no_grad():
                img_in, _, _ = extractor.preprocess(img.unsqueeze(0))
                out = extractor.dino(pixel_values=img_in)
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            "point_times": self.scene_times, # [N, 1] 原始点的时间戳
